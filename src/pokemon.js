...
const META = {
  bulbasaur: { beh: 'wander', flee: 6, size: 1.4 }, charmander: { beh: 'approach', flee: 9, size: 1.4 },
  squirtle: { beh: 'wander', flee: 6, size: 1.4 }, pikachu: { beh: 'approach', flee: 9, size: 1.3 },
  eevee: { beh: 'flee', flee: 9, size: 1.3 }, jigglypuff: { beh: 'wander', flee: 7, size: 1.2 },
  meowth: { beh: 'flee', flee: 8, size: 1.2 }, psyduck: { beh: 'wander', flee: 6, size: 1.4 },
  oddish: { beh: 'flee', flee: 7, size: 0.9 }, geodude: { beh: 'wander', flee: 5, size: 1.2 },
  magikarp: { beh: 'wander', flee: 5, size: 1.4 }, pidgey: { beh: 'flee', flee: 8, size: 1.0 },
  rattata: { beh: 'flee', flee: 7, size: 0.95 }, caterpie: { beh: 'wander', flee: 5, size: 0.9 },
  zubat: { beh: 'flee', flee: 8, size: 0.95 }, diglett: { beh: 'wander', flee: 5, size: 0.9 },
  clefairy: { beh: 'approach', flee: 9, size: 1.3 }, vulpix: { beh: 'flee', flee: 8, size: 1.3 },
};

export function createPokemon(scene, ctx) {
  const texCache = {};
  function texFor(name) { if (texCache[name]) return texCache[name]; const t = new B.Texture('/pokemon/' + name + '.png', scene, false, true, B.Texture.BILINEAR_SAMPLINGMODE); t.hasAlpha = true; texCache[name] = t; return t; }
  function makeMat(name) {
    ...
    m.roughness = 1; m.metallic = 0; m.backFaceCulling = false; m.emissiveColor = new B.Color3(0.12, 0.12, 0.13);
    return m;
  }
  // shared blob shadow
  ...
  for (let i = 0; i < COUNT; i++) {
    ...
    plane.billboardMode = B.Mesh.BILLBOARDMODE_ALL; plane.material = makeMat(name);
    plane.position.set(hx, height(hx, hz) + size * 0.52, hz); plane.applyFog = true; plane.isPickable = false;
    ...
    list.push({ name, meta, plane, shadow: sh, hx, hz, dir: Math.random() * 6.28, speed: 0.6 + Math.random() * 0.5, stateT: Math.random() * 3, moving: false, size });
  }
  ...
  function update(dt, player) {
    for (const p of list) {
      ...
      p.plane.position.y = gy + p.size * 0.52 + Math.sin(performance.now() * 0.003 + p.hx) * 0.04;
      // grounded blob shadow
      p.shadow.position.set(p.plane.position.x, gy + 0.04, p.plane.position.z);
      p.shadow.scaling.set(1, 0.6, 1);
    }
  }
  return { list, update };
}
