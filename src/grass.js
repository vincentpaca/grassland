...
// World-anchored swaying grass (thin instances). Blades stay put in the world;
// only out-of-range blades recycle to the ring edge as you walk — no sliding carpet.
...
export function createGrass(scene, ctx) {
  ...
  for (let i = 0; i < p.length / 3; i++) p[i * 3 + 1] += 0.35; // pivot at base
  ...
  mat.opacityTexture = tex; mat.transparencyMode = B.Material.MATERIAL_ALPHATEST;
  ...
  for (let i = 0; i < COUNT; i++) {
    ...
    fx[i] = Math.cos(a) * r; fz[i] = Math.sin(a) * r;
  ...
  function update(playerPos, wind) {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      if (ddx * ddx + ddz * ddz > R2) {
        // recycle to ring edge, biased in the direction of travel so new grass grows ahead
